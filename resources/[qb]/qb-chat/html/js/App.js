window.APP = {
  ...
  mounted() {
    post('http://md-chat/loaded', JSON.stringify({}));
  ...
  methods: {
    ...
    keyDown(e) {
      ...
      if (clickSound) {
        clickSound.pause(); // Stop the current sound
        clickSound.currentTime = 0; // Reset the sound to the beginning
        clickSound.play(); // Play the sound again
      }
    ...
    send(e) {
      if(this.message !== '') {
        post('http://md-chat/chatResult', JSON.stringify({
    ...
    hideInput(canceled = false) {
      if (canceled) {
        post('http://md-chat/chatResult', JSON.stringify({ canceled }));
      }
      this.message = '';
    ...
    completeSuggestion() {
      ...
      if(topSuggestion) {
        this.message = topSuggestion.name;
      }
    },
...
// print("md-chat loaded successfully");
