...
PlayerData = QBCore.Functions.GetPlayerData() or {} -- Setting this for when you restart the resource in game
...
local function IsPoliceOrEMS()
    return PlayerData and PlayerData.job and (PlayerData.job.name == 'police' or PlayerData.job.type == 'leo' or PlayerData.job.name == 'ambulance')
end
...
RegisterCommand('radialmenu', function()
    local isCuffed = PlayerData and PlayerData.metadata and PlayerData.metadata['ishandcuffed']
    if ((IsDowned() and IsPoliceOrEMS()) or not IsDowned()) and not isCuffed and not IsPauseMenuActive() and not inRadialMenu then
